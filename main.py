...
# 時間框架
TIMEFRAME      = '4h'   # 改為 4H，與主版本一致
MAX_HOLD_HOURS = 48     # 4H 週期持倉時間拉長
...
async def monitor_async():
    ...
    while True:
        try:
            ...
            async def process(symbol):
                async with semaphore:
                    try:
                        ...
                        if pos:
                            side, ep, sl, tp1, tp2 = (
                                pos['side'], pos['ep'], pos['sl'],
                                pos['tp1'], pos['tp2']
                            )
                            # 止損用最高最低價，止盈用收盤價
                            sl_triggered = (
                                (side == 'long'  and curr_l <= sl) or
                                (side == 'short' and curr_h >= sl)
                            )
                            exit_reason = None
                            exit_c      = sl if sl_triggered else curr_c

                            if sl_triggered:
                                exit_reason = "止損"
                            elif (side == 'long'  and curr_c >= tp2) or \
                                 (side == 'short' and curr_c <= tp2):
                                exit_reason = "止盈2"
                            elif not pos['tp1_hit'] and (
                                (side == 'long'  and curr_c >= tp1) or
                                (side == 'short' and curr_c <= tp1)
                            ):
                                update_position(symbol, {'tp1_hit': True, 'sl': ep})
                                send_telegram(
                                    f"✅ *V2-Pro {symbol} 第一批止盈*\n"
                                    f"進場: {ep:.4f} → 現價: {curr_c:.4f}\n"
                                    f"🛡 止損已移至保本價"
                                )
                            ...
                            if not exit_reason and \
                               (time.time() - entry_time) / 3600 >= MAX_HOLD_HOURS:
                                exit_reason = "超時"
                                exit_c      = curr_c

                            if exit_reason:
                                pnl = (exit_c/ep - 1)*100 if side == 'long' \
                                      else (ep/exit_c - 1)*100
                                send_telegram(
                                    f"🏁 *V2-Pro {symbol} 出場 ({exit_reason})*\n"
                                    f"損益: `{pnl:+.2f}%`\n"
                                    f"進場: `{ep:.4f}` → 出場: `{exit_c:.4f}`"
                                )
            ...
            # 對齊 4H 收盤（UTC 00/04/08/12/16/20 + 2分鐘緩衝）
            now       = datetime.datetime.now(datetime.timezone.utc)
            ...
            if next_4h >= 24:
                ...
                next_dt = now.replace(
                    hour=next_4h, minute=2, second=0, microsecond=0)
    ...
    while True:
        try:
            ...
            if now.date() > last_sent_date:
                ...
                if 'total_pnl' not in all_s: all_s['total_pnl'] = 0
                pos_detail = '\n'.join([
                    f"  • `{sym}` {p['side']} @ {p['ep']:.4f} "
                    f"({'已TP1' if p.get('tp1_hit') else '持倉中'}) "
...
async def main():
    ...
    send_telegram(
        ...
        f"✅ RSI背離過濾: 頂/底背離不進場\n"
        f"✅ BB寬度過濾: 低波動期不進場\n"
        f"✅ 結構止損: 近{BREAKOUT_LOOKBACK}根低/高點 + ATR buffer\n"
        f"✅ 止損判斷: 最高最低價（影線即觸發）\n{'─'*24}\n"
