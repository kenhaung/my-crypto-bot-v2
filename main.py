...
async def check_market_regime(exchange, symbol: str = 'BTC/USDT') -> str:
    ...
    try:
        ...
        for i in range(len(df)):
            try:
                close_list.append(float(df['c'].iloc[i]))
                high_list.append(float(df['h'].iloc[i]))
                low_list.append(float(df['l'].iloc[i]))
            except:
                continue
        ...
        # 計算 ADX
        try:
            adx_result = ta.adx(high_series, low_series, close_series, length=14)
            if adx_result is not None and len(adx_result) > 0:
                ...
                if adx_col:
                    ...
                    adx_value = float(adx_val) if not pd.isna(adx_val) else 0
                else:
                    adx_value = 0
            else:
                adx_value = 0
        except Exception as e:
            log.debug(f"ADX 計算失敗: {e}")
            adx_value = 0
        ...
        try:
            ...
            log.debug(f"EMA 計算失敗: {e}")
            ema_20_val = 0
            ema_50_val = 0
        
        # 判斷趨勢
        if adx_value > 25 and ema_20_val > ema_50_val and ema_20_val > 0 and ema_50_val > 0:
            return 'trend_up'
        elif adx_value > 25 and ema_20_val < ema_50_val and ema_20_val > 0 and ema_50_val > 0:
            return 'trend_down'
        else:
            return 'ranging'
...
async def multi_timeframe_confirmation(exchange, symbol: str) -> str:
    ...
    for tf in timeframes:
        try:
            ...
            ema_fast_val = float(ema_fast.iloc[-1]) if ema_fast is not None and len(ema_fast) > 0 and not pd.isna(ema_fast.iloc[-1]) else 0
            ema_slow_val = float(ema_slow.iloc[-1]) if ema_slow is not None and len(ema_slow) > 0 and not pd.isna(ema_slow.iloc[-1]) else 0
            
            if ema_fast_val > 0 and ema_slow_val > 0:
                trend = 'bull' if ema_fast_val > ema_slow_val else 'bear'
...
async def adaptive_parameters(exchange, symbol: str) -> dict:
    ...
    try:
        ...
        for i in range(len(atr)):
            if i < len(close_series) and close_series.iloc[i] > 0:
                atr_val = float(atr.iloc[i]) if not pd.isna(atr.iloc[i]) else 0
                if atr_val > 0:
                    atr_pct = atr_val / close_series.iloc[i] * 100
                    atr_pct_list.append(atr_pct)
        
        if len(atr_pct_list) == 0:
            return {}
        
        current_atr_pct = atr_pct_list[-1] if atr_pct_list else 0
        percentile_rank = sum(1 for x in atr_pct_list if x < current_atr_pct) / len(atr_pct_list) if atr_pct_list else 0.5
        ...
        log.debug(f"自適應參數計算失敗 ({symbol}): {e}")
        return {}
    ...
    try:
        ...
        for i in range(len(df)):
            try:
                ...
                high_list.append(float(df['h'].iloc[i]))
                low_list.append(float(df['l'].iloc[i]))
                open_list.append(float(df['o'].iloc[i]))
                vol_list.append(float(df['v'].iloc[i]))
            except:
                continue
        ...
        high_series = pd.Series(high_list)
        low_series = pd.Series(low_list)
        open_series = pd.Series(open_list)
        ...
        # 計算指標
        try:
            ema_fast = ta.ema(close_series, length=EMA_FAST_LENGTH)
            ema_mid = ta.ema(close_series, length=EMA_MID_LENGTH)
            ema_slow = ta.ema(close_series, length=EMA_SLOW_LENGTH)
            
            ema_fast_val = float(ema_fast.iloc[-1]) if ema_fast is not None and len(ema_fast) > 0 and not pd.isna(ema_fast.iloc[-1]) else 0
            ema_mid_val = float(ema_mid.iloc[-1]) if ema_mid is not None and len(ema_mid) > 0 and not pd.isna(ema_mid.iloc[-1]) else 0
            ema_slow_val = float(ema_slow.iloc[-1]) if ema_slow is not None and len(ema_slow) > 0 and not pd.isna(ema_slow.iloc[-1]) else 0
        except:
            ema_fast_val = ema_mid_val = ema_slow_val = 0
        
        try:
            rsi = ta.rsi(close_series, length=RSI_LENGTH)
            rsi_val = float(rsi.iloc[-1]) if rsi is not None and len(rsi) > 0 and not pd.isna(rsi.iloc[-1]) else 50
        except:
            rsi_val = 50
        
        try:
            atr = ta.atr(high_series, low_series, close_series, length=ATR_LENGTH)
            atr_val = float(atr.iloc[-1]) if atr is not None and len(atr) > 0 and not pd.isna(atr.iloc[-1]) else 0
        except:
            atr_val = 0
        ...
        try:
            ...
            if bb is not None and len(bb) > 0:
                upper = bb.iloc[:, 0] if hasattr(bb, 'columns') else bb
                lower = bb.iloc[:, 1] if hasattr(bb, 'columns') and len(bb.columns) > 1 else bb
                middle = bb.iloc[:, 2] if hasattr(bb, 'columns') and len(bb.columns) > 2 else bb
                
                if len(upper) > 0 and len(lower) > 0 and len(middle) > 0:
                    bb_width = (float(upper.iloc[-1]) - float(lower.iloc[-1])) / float(middle.iloc[-1])
                    bb_width_history = []
                    for j in range(len(upper)):
                        if j < len(lower) and j < len(middle):
                            up_val = float(upper.iloc[j]) if not pd.isna(upper.iloc[j]) else 0
                            low_val = float(lower.iloc[j]) if not pd.isna(lower.iloc[j]) else 0
                            mid_val = float(middle.iloc[j]) if not pd.isna(middle.iloc[j]) else 0
                            if mid_val > 0:
                                bb_width_history.append((up_val - low_val) / mid_val)
                    
                    if len(bb_width_history) > 0:
                        threshold = np.percentile(bb_width_history, BB_WIDTH_PERCENTILE)
                        if bb_width < threshold:
                            return None
        except Exception as e:
            log.debug(f"BB 計算失敗: {e}")
        
        # 成交量確認
        vol_start = max(0, len(vol_series) - VOL_LOOKBACK - 1)
        vol_end = len(vol_series) - 1
        if vol_end > vol_start:
            avg_vol = float(vol_series.iloc[vol_start:vol_end].mean())
        else:
            avg_vol = float(vol_series.mean())
        
        # 突破條件
        high_start = max(0, len(high_series) - BREAKOUT_LOOKBACK - 1)
        high_end = len(high_series) - 1
        low_start = max(0, len(low_series) - BREAKOUT_LOOKBACK - 1)
        low_end = len(low_series) - 1
        
        recent_high = float(high_series.iloc[high_start:high_end].max())
        recent_low = float(low_series.iloc[low_start:low_end].min())
